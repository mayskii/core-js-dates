...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const diffTime = new Date(dateEnd) - new Date(dateStart);
...
function getNextFridayThe13th(date) {
  ...
  if (current.getDay() === 5 && current > date) {
    return new Date(current);
  }
  const maxDate = new Date(date.getFullYear() + 100, date.getMonth(), 13);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const current = toDate(period.start);
  const end = toDate(period.end);
  const schedule = [];

  while (current <= end) {
    for (let i = 0; i < countWorkDays && current <= end; i += 1) {
      schedule.push(formatDateLocal(new Date(current)));
      current.setDate(current.getDate() + 1);
    }
    for (let i = 0; i < countOffDays && current <= end; i += 1) {
      current.setDate(current.getDate() + 1);
    }
  }
  return schedule;
